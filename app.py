# Talep Tahminleme + Kapasite Planlama (Flask + MySQL)
...
# ---- MySQL ayarları (Railway Variables'tan) ----
...
def fit_predict_lr(y: np.ndarray, h: int):
    n = len(y); X = np.arange(n).reshape(-1, 1)
...
def run_pipeline(xlsx_file, capfile, h, col_date, col_demand, col_depo, col_urun, wma_window):
    ...
    for urun, g in agg.groupby("urun", as_index=False):
        ...
        out = pd.DataFrame({"tarih": fut_idx, "urun": urun, "linreg": lr, "wma": wma})
        out["tahmin_mean"] = out[["linreg", "wma"]].mean(axis=1)
        outs.append(out)
    if not outs: abort(400, "Yeterli veri yok veya 'urun' sütunu boş.")
...
def save_results_to_db(run_id: str, df: pd.DataFrame):
    ...
    for _, r in df.iterrows():
        rows.append((
            ...
            None if "kapasite" not in df.columns or pd.isna(r.get("kapasite")) else float(r["kapasite"]),
...
def forecast_endpoint():
    ...
    try:
        ...
        # DB yazımı opsiyonel; hata olursa sadece logla
...
def get_run(run_id):
    with db() as c, c.cursor() as cur:
        cur.execute("""
            ...
            FROM forecast_results WHERE run_id=%s ORDER BY tarih
        """, (run_id,))
        rows = cur.fetchall()
...
def download_last():
    ...
    return send_file(io.BytesIO(_LAST_CSV_BYTES),
        mimetype="text/csv", as_attachment=True, download_name="forecast_capacity.csv")

@app.get("/health")
def health():
    # DB ping
    try:
        with db() as c: c.ping(reconnect=True)
        db_ok = True
    except Exception:
        db_ok = False
